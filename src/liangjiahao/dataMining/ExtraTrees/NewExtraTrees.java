package liangjiahao.dataMining.ExtraTrees;
...
public class NewExtraTrees {
    ...
    public NewExtraTrees(String filePath) {
    ...
    class TreeNode {
        ...
        //  arr[1-2] is the gini,arr[3] is the bandary,arr[4] is the col
            ...
            for(int j=0,n=data.get(0).size();j<n;j++) { //for every characher
                ...
                double bmin = newArr.get(0);
                double bmax = newArr.get(newArr.size()-1);
                double bandary=(bmax-bmin)*random.nextDouble()+bmin;
                if(getGini(bandary,j)[2]<minmin){
                    resultArr = getGini(bandary,j);
                    minmin = resultArr[2];
                    resultArr[4] = j;
                    resultArr[3] = bandary;
                }
